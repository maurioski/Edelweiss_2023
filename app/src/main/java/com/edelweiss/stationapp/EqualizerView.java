...
public class EqualizerView extends LinearLayout {
    ...
    ArrayList<Animator> mAnimatorPlay = new ArrayList<>();
    ArrayList<Animator> mAnimatorStop = new ArrayList<>();
    ...
    private void initView() {
        ...
        for (int i = 0; i < mBarCount; i++) {
            ...
            LayoutParams params = new LayoutParams(mBarWidth, ViewGroup.LayoutParams.MATCH_PARENT);
    ...
    public void animateBars() {
        ...
        if (isInBatterySaveMode()) {
            if (mRunInBatterySafeMode) {
                mThreadAnimation=new Thread(mAnimationRunnable);
            ...
            if (mPlayingSet == null) {
                for (int i = 0; i < mBars.size(); i++) {
                    ...
                    mAnimatorPlay.add(scaleYbar);
                }
                mPlayingSet = new AnimatorSet();
                mPlayingSet.playTogether(mAnimatorPlay);
    ...
    public void stopBars() {
        ...
        if (!isInBatterySaveMode()) {
            ...
            if (mStopSet == null) {
                mAnimatorStop.clear();
                for (int i = 0; i < mBars.size(); i++) {
                    mAnimatorStop.add(ObjectAnimator.ofFloat(mBars.get(i), "scaleY", 0.1f));
                }
                mStopSet = new AnimatorSet();
                mStopSet.playTogether(mAnimatorStop);
            ...
            if (mRunInBatterySafeMode) {
                if(mThreadAnimation!=null){
                    mThreadAnimation.interrupt();
                    mThreadAnimation=null;
                }
                resetView();
                initView();
            }
    ...
    public void onDestroy(){
        try{
            ...
            if(mAnimatorPlay !=null){
                mAnimatorPlay.clear();
                mAnimatorPlay =null;
            }
            if(mAnimatorStop !=null){
                mAnimatorStop.clear();
                mAnimatorStop =null;
            }
            if(mPlayingSet!=null){
                ...
                for (int i = 0; i < mBars.size(); i++) {
                    ...
                    bar_tmp.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            LayoutParams params = (LayoutParams) bar_tmp.getLayoutParams();
